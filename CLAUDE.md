...
- **`docs/roadmap.md`** — 「今日のページ」完成形のSoT。シート構造／パイプライン／カード仮称／ピン留め事項を記載
- 同ファイル冒頭の「🤖 私（Claude）の自動化可能範囲」を必ず確認。**clasp push/deploy・本番URL経由のサーバー実行・画像目視評価まで自力で完結する**こと。「確認していい？」「これでいい？」は設計判断レベル以外で聞かない
