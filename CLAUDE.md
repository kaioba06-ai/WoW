...
- `index.html` を編集するのは、ヘッダー・ナビゲーション・モーダル・グローバル CSS・JavaScript のみです。
- すべての JS ロジックは `index.html` 末尾のインライン `<script>` ブロックに記述されています（`js/` フォルダのファイルは UI ユーティリティのみ）。

### 読み込み仕組み

`window.onload` 内の `loadSections()` が `fetch()` + `createContextualFragment()` で各 `*.html` を動的に読み込みます。
